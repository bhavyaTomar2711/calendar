    ...
    <div className="flex flex-col h-full overflow-hidden overflow-x-hidden bg-white dark:bg-[#1f1f1f]">
      ...
      <div className="flex shrink-0 bg-white dark:bg-[#1f1f1f] pt-4 pb-3">
        ...
        <div className="flex-1 text-center py-3 border-l border-[#e8eaed] dark:border-[#3d3d3d] min-h-[80px] flex flex-col items-center justify-center">
          <div className={`text-[11px] font-medium uppercase tracking-wide mb-1 ${isToday ? 'text-[#1a73e8]' : 'text-[#70757a] dark:text-gray-400'}`}>
            {format(currentDate, 'EEEE')}
          </div>
          <div
            className={`text-[40px] font-light mx-auto flex items-center justify-center rounded-full
              ${isToday ? 'bg-[#1a73e8] text-white w-14 h-14 font-medium' : 'text-[#3c4043] dark:text-gray-200 w-14 h-14'}`}
          >
